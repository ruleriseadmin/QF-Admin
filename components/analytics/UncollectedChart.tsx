    ...
    useEffect(() => {
      ...
      const fetchData = async () => {
        ...
        queryObj.end_date   = formatDate(end);   // ← last day of month
          ...
          const response = await apiClient.get(`/analytics/uncollected?${queryString}`, {
...
const formattedData = chartData?.map(item => ({
  ...item,
  loan_disbursed: Math.abs(item.loan_disbursed), 
  loan_open: Math.abs(item.loan_open),
  loan_overdue: Math.abs(item.loan_overdue),
}));

const pieColor = ['#30353E', '#2B6E53','#DA3737'];
const names = ['Total Disbursed', 'Amount Open','Amount Overdue'];


// Ensure chartData is defined and not empty
const validChartData = Array.isArray(formattedData) ? formattedData : [];

const loan_disbursed = validChartData.reduce(
  (acc, item) => acc + (item?.loan_disbursed || 0), 
  0
);
const loan_open = validChartData.reduce(
  (acc, item) => acc + (item?.loan_open || 0), 
  0
);
const loan_overdue = validChartData.reduce(
  (acc, item) => acc + (item?.loan_overdue || 0), 
  0
);
...
const collectedUsers = validChartData.reduce(
  (acc, item) => acc + (item?.loan_open_users_count || 0), 
  0
);
const unCollectedUsers = validChartData.reduce(
  (acc, item) => acc + (item?.loan_overdue_users_count || 0), 
  0
);

const pieData = names?.map((item, index) => ({
  name: item,
  value: index === 0 ? loan_disbursed : index === 1 ? loan_open : loan_overdue,
  color: pieColor[index],
  users: index === 0 ? disbursedUsers : index === 1 ? collectedUsers : unCollectedUsers,
}));
        ...
        <div className=" h-[516px] lg:w-full md:w-[810px] w-[810px]   bg-[#FFFFFF]  shadow-custom2 rounded-[22px]">
          <div className="flex justify-start items-center mt-8 px-4 md:pt-4 pt-4">
            ...
            <div className="flex items-center gap-4 text-[#464646] font-medium text-[14px]">
              ...
              <p className="flex items-center gap-1">
                <span className="w-3 h-3 rounded-full bg-[#2B6E53]"></span>
                <span>Amount Open</span>
              </p>
              <p className="flex items-center gap-1">
                <span className="w-3 h-3 rounded-full bg-[#DA3737]"></span>
...
/>
        ...
        <div className="lg:col-span-3 md:mt-6 w-full h-[516px] lg:mt-8 mt-8 bg-[#FFFFFF] shadow-custom2 rounded-[22px]">
          <p className='text-center text-[14px] font-semibold text-[#5A5A5A] px-4 pt-6'>TOTAL DISBURSEMENT, OPEN & OVERDUE
...
export default UncollectedChart
