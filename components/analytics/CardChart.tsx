  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      queryObj.end_date   = formatDate(end);   // ← last day of month
        ...
        const response = await apiClient.get(`/analytics/card-users?${queryString}`, {
...
const formattedData = chartData.map(item => ({
  ...item,
  registration_count: Math.abs(item.registration_count), 
  card_count: Math.abs(item.card_count)
}));

const pieColor = ['#2B6E53', '#A9CFC0'];
const names = ['REGISTRATION', 'CARD TOKENIZATION'];

// Ensure chartData is defined and not empty
const validChartData = Array.isArray(formattedData) ? formattedData : [];


const regUsers = validChartData.reduce(
  (acc, item) => acc + (item?.registration_count || 0), 
  0
);

const cardUsers = validChartData.reduce(
  (acc, item) => acc + (item?.card_count || 0), 
  0
);

const pieData = names.map((item, index) => ({
  name: item,
  users: index === 0 ? regUsers : cardUsers,
        ...
        <div className=" h-[516px] lg:w-full md:w-[810px] w-[810px]   bg-[#FFFFFF]  shadow-custom2 rounded-[22px]">
          <div className="flex justify-between items-center mt-8 px-4 md:pt-4 pt-4">
            <div className="flex items-center font-semibold text-[#282828] text-[16px] gap-2 ">
              ...
              <p>REGISTRATION VS CARD TOKENIZATION</p>
            </div>
            <div className="flex items-center gap-4 text-[#464646] font-medium text-[14px]">
              ...
              <p className="flex items-center gap-1">
                <span className="w-3 h-3 rounded-full bg-[#A9CFC0]"></span>
                <span>Card Tokenization</span>
              </p>
...
/>
        ...
        <div className="lg:col-span-3 md:mt-6 w-full h-[516px] lg:mt-8 mt-8  bg-[#FFFFFF] shadow-custom2 rounded-[22px]">
          <p className='text-center text-[14px] font-semibold text-[#5A5A5A] px-4 pt-6'>REGISTRATION VS CARD TOKENIZATION
          ( P & I )
          </p>
